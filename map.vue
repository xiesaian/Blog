...
export default {
  ...
  mounted() {
    ...
    if (modelId) {
      ...
      // 注册到全局供 krpano 调用
      window.onCameraClicked = function (cameraId) {
      ...
      window.onPictureClicked = function (url) {
        that.previewList = []
        if (url) {
  ...
  beforeDestroy() {
    ...
    this.destroyAllMapResources({
      releaseLargeData: true,
      destroyKrpano: true,
      clearWindowCallbacks: true,
    })
  },
  methods: {
    _safeTimeout(fn, delay) {
      const id = setTimeout(() => {
        const idx = this._timerIds.indexOf(id)
        if (idx > -1) {
          this._timerIds.splice(idx, 1)
        }
    ...
    clearPendingTimers() {
      if (this._timerIds && this._timerIds.length) {
    ...
    destroyCesiumViewer() {
      ...
      try {
        const isDestroyed =
          typeof this.viewer.isDestroyed === 'function'
            ? this.viewer.isDestroyed()
            : false
        if (!isDestroyed) {
          this.viewer.scene && this.viewer.scene.primitives.removeAll()
          this.viewer.destroy()
        }
      } catch (e) {
        console.warn('destroyCesiumViewer failed', e)
      }
      this.viewer = null
      const cesiumEl = document.getElementById('cesium01')
      if (cesiumEl) {
        cesiumEl.innerHTML = ''
      }
    },
    destroyFlvPlayer() {
      ...
      try {
        ...
        console.warn('destroyFlvPlayer failed', e)
      }
      this.flvPlayer = null
    },
    destroyKrpanoResources() {
      if (window.krpano && typeof window.krpano.removepano === 'function') {
        window.krpano.removepano()
      }
    ...
    destroyAllMapResources(options = {}) {
      const {
        releaseLargeData = false,
        destroyKrpano = false,
        clearWindowCallbacks = false,
      } = options
      ...
      this.clearMapCanvases()
      this.showClip = false

      if (this.gMap) {
        try {
          this.gMap.markerLayer && this.gMap.markerLayer.removeAllMarkers()
          ...
          console.warn('destroy gMap failed', e)
        }
        this.gMap = null
      }
      this.textLayer = null
    ...
    goToMapDetail() {
      ...
      if (hasItemWithName) {
        ...
        localStorage.setItem('hasChooseNode', 'false')
        this.$router.push({
          path: '/video-manager/real-time',
